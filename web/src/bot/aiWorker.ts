...
import init, { omok_best_move, chess_ai, checkers_ai } from "../wasm-ai/ai_wasm.js";
...
type Req =
  ...
  | { id: number; kind: "checkers"; pos: string; level: number };
...
self.onmessage = async (e: MessageEvent<Req>) => {
  ...
  } else if (m.kind === "checkers") {
    res = checkers_ai(m.pos, m.level);
  } else {
