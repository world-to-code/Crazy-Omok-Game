...
홈에서 오목/체스를 고른 뒤 **🤖 봇과 대결** → 난이도(쉬움·중간·어려움·**헬**) + 선공 선택 → 즉시 시작. 방·서버 없이 **브라우저 안에서만** 진행됩니다.

- AI 엔진은 **Rust → WebAssembly** (`web/wasm-ai/`).
  - **오목**: 윈도우 패턴 평가 + 반복심화 알파베타(즉승/즉방 전술) + **VCF 완전탐색**(연속 4로 강제승 수순 발견).
  - **체스**: 규칙 권위 판정 + 반복심화 알파베타(Zobrist **치환표**, MVV-LVA 정렬, 정지탐색, 체크 연장, LMR) + 평가(재료·PST·캐슬링권·킹 안전·폰 구조) + **오프닝북**(정석 무작위).
- 무거운 탐색은 **Web Worker**(`web/src/bot/aiWorker.ts`)에서 돌려 화면이 멈추지 않습니다. 사고 시간: 쉬움 ≈0.1초, 중간 ≈0.9초, 어려움 최대 5초, **헬 최대 10초**(더 깊고 넓게 탐색). 차례 제한시간 45초는 표시용(강제 없음).
